...
export class DukCallStackEntry {
    ...
    public constructor(fileName: string, funcName: string, lineNumber: number, pc: number) {
...
class DukMsgBuilder {
    ...
    public writeString(val: string): void {
        assert(typeof val === "string" || typeof val === "undefined");
...
class PendingRequest {
    ...
    constructor(cmd: number, sequence: number, promise: Promise<any>, pcontext: any, buf: Buffer) {
        this.cmd = cmd;
        this.sequence = sequence;
...
export class DukDbgProtocol extends EE.EventEmitter {
    ...
    private sendRequest(cmd: number, buf: Buffer): Promise<any> {
        ...
        let req = new PendingRequest(cmd, ++this._requestSequence, p, pcontext, buf);
    ...
    private translateMessage(msg: DukDvalueMsg): void {
        ...
        if (MSG_TRACING) {
            if (LOG_STATUS_NOTIFY || (msg[0].value !== Duk.MsgType.NFY || msg[1].value !== Duk.NotifyType.STATUS)) {
                let mStr = "IN <- ";
    ...
    private parseResponseMessage(msg: DukDvalueMsg): void {
        ...
        if (msg[0].value === Duk.MsgType.ERR) {
            ...
            let errStr = `Request ${Duk.MsgType[cmd]} returned error: ` + `${Duk.ERR_TYPE_MAP[errType]} : ${errMsg}`;
